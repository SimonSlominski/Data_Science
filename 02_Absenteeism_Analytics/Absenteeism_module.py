...
class CustomScaler(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.var_ = no.array(np.var(X[self.columns]))
        return self
...
class absenteeism_model():
    ...
    def load_and_clean_data(self,data_file):
        ...
        reason_type_1 = reason_colums.loc[:,1:14].max(axis=1)
        reason_type_2 = reason_colums.loc[:,15:17].max(axis=1)        
        reason_type_3 = reason_colums.loc[:,18:21].max(axis=1)       
        reason_type_4 = reason_colums.loc[:,22:].max(axis=1) 
